...
GRUB_CFG=${BUILDROOT}/boot/grub2/grub.cfg
if [ -f "$GRUB_CFG" ]; then
  mv "$GRUB_CFG" "${GRUB_CFG}.bak"
fi

cat > "$GRUB_CFG" << EOF
# Begin /boot/grub2/grub.cfg

# Generated by NiceOS Installer
...
set theme=${BOOT_DIR}/grub2/themes/niceos/theme.txt

load_env -f ${BOOT_DIR}/niceos.cfg
...
menuentry "NiceOS" {
  linux ${BOOT_DIR}/\$niceos_linux root=\$rootpartition \$niceos_cmdline \$systemd_cmdline ${POI_CMDLINE} \$user_cmdline ${EXTRA_PARAMS}

  if [ -f ${BOOT_DIR}/\$niceos_initrd ]; then
    initrd ${BOOT_DIR}/\$niceos_initrd
  fi
}
...
if [ -n "$OTHER_ENTRIES_FILE" ] && [ -f "$OTHER_ENTRIES_FILE" ]; then
  cat "$OTHER_ENTRIES_FILE" >> "$GRUB_CFG"
fi
